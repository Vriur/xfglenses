#include "../headers/uniformringmodel.h"

UniformRingModel::UniformRingModel(){
    this->setName("Uniform Ring");
    ...
    this->parameters.push_back(new ModelParameter("Ri", 1.00, 0.00, 2.00));
    this->parameters.push_back(new ModelParameter("Ro", 2.00, 0.00, 4.00));
}

double UniformRingModel::jacobian(double x, double y){
    return x + y;
}

void UniformRingModel::setAlphas(double x1, double x2){
    alpha1 = x1 + x2;
}
